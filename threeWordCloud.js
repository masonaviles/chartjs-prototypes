// threeWordCloud.js

const wordCloudContainer = document.getElementById("wordCloudContainer");
const cloudScene = new THREE.Scene();

// Camera setup
const cloudCamera = new THREE.PerspectiveCamera(
  50,
  wordCloudContainer.clientWidth / wordCloudContainer.clientHeight,
  0.1,
  1000
);
cloudCamera.position.z = 15; // move it closer (was 30 before)

// Renderer
const cloudRenderer = new THREE.WebGLRenderer({ alpha: true, antialias: true });
cloudRenderer.setSize(
  wordCloudContainer.clientWidth,
  wordCloudContainer.clientHeight
);
cloudRenderer.setPixelRatio(window.devicePixelRatio);
wordCloudContainer.appendChild(cloudRenderer.domElement);

// Lighting
const cloudLight = new THREE.PointLight(0xffffff, 1);
cloudLight.position.set(10, 10, 10);
cloudScene.add(cloudLight);

// Font material (glowy softer look)
const textMaterial = new THREE.MeshBasicMaterial({
  color: 0x66ccff,
  transparent: true,
  opacity: 0.9,
});

// Word list
const words = [
  "State Park",
  "Hiking Area",
  "Historical Landmark",
  "Public Beach",
  "Campground",
  "Fishing Area",
  "Picnic Ground",
  "Vista Point",
  "National Forest",
];

// Load font and create 3D texts
const loader = new THREE.FontLoader();
loader.load(
  "https://threejs.org/examples/fonts/helvetiker_regular.typeface.json",
  (font) => {
    words.forEach((word, idx) => {
      const textGeometry = new THREE.TextGeometry(word, {
        font: font,
        size: 0.8, // small enough to fit, but visible now
        height: 0.1,
        curveSegments: 12,
      });

      const textMesh = new THREE.Mesh(textGeometry, textMaterial.clone());

      textMesh.position.x = (Math.random() - 0.5) * 10;
      textMesh.position.y = (Math.random() - 0.5) * 5;
      textMesh.position.z = (Math.random() - 0.5) * 6;

      textMesh.rotation.y = Math.random() * Math.PI * 2;
      textMesh.rotation.x = Math.random() * Math.PI * 2;

      animateText(textMesh);

      cloudScene.add(textMesh);
    });

    animate();
  }
);

// Animate each word
function animateText(mesh) {
  gsap.to(mesh.position, {
    x: `+=${(Math.random() - 0.5) * 1.5}`,
    y: `+=${(Math.random() - 0.5) * 1.5}`,
    z: `+=${(Math.random() - 0.5) * 1.5}`,
    duration: 4 + Math.random() * 4,
    repeat: -1,
    yoyo: true,
    ease: "sine.inOut",
  });
}

// Mouse follow setup
const mouse = { x: 0, y: 0 };
window.addEventListener("mousemove", (event) => {
  mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
  mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
});

// Animate scene
function animate() {
  requestAnimationFrame(animate);

  cloudCamera.position.x += (mouse.x * 5 - cloudCamera.position.x) * 0.05;
  cloudCamera.position.y += (mouse.y * 5 - cloudCamera.position.y) * 0.05;
  cloudCamera.lookAt(cloudScene.position);

  cloudRenderer.render(cloudScene, cloudCamera);
}

// Handle window resize
window.addEventListener("resize", () => {
  cloudCamera.aspect =
    wordCloudContainer.clientWidth / wordCloudContainer.clientHeight;
  cloudCamera.updateProjectionMatrix();
  cloudRenderer.setSize(
    wordCloudContainer.clientWidth,
    wordCloudContainer.clientHeight
  );
});
